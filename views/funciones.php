...
define("Tienda", true);
define("Inmobiliaria", true);



 
if( empty($_GET["bq"]) )$_GET["bq"] = "mis-datos";
if( !isset($_SESSION["LoginAcceso"]) && $_GET["bq"]!="ingreso") header("Location: index.php?bq=ingreso");


if($_GET["bq"] == "salir" || !isset($_GET["bq"])){
      if( isset($_SESSION["LoginAcceso"]) ){ 
            session_unset();
            session_destroy();
            header("Location: index.php?bq=ingreso");
      } 
}
...
function ActiveMenu($get, $serch){
 
    $dat = explode(",",$serch); 
     
    for ($i=0; $i <= count($dat) ; $i++) {  
        if(!empty($dat[$i]) ){
                  $posicion_coincidencia = strpos($get, $dat[$i]);
